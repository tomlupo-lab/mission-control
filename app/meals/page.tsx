      ...
      <CardContent style={{ padding: "var(--space-lg)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontWeight: 700, fontSize: "0.85rem", color: isToday ? "var(--accent-hex)" : "var(--text)" }}>
              {day.day}
            </span>
            {day.isFish && <span style={{ fontSize: "0.75rem" }}>🐟</span>}
            {isToday && (
              <span style={{ fontSize: "0.6rem", padding: "1px 6px", borderRadius: 8, background: "var(--accent-dim)", color: "var(--accent-hex)", fontWeight: 600 }}>
                TODAY
              </span>
            )}
          </div>
          <div style={{ fontSize: "0.75rem", textAlign: "right" }}>
            {hasLog ? (
              ...
              <span style={{ color: "var(--muted-hex)" }}>{day.totalKcal} kcal</span>
            )}
          </div>
        </div>
        ...
        {/* Expanded content */}
        {expanded && (
          <div style={{ marginTop: 12, borderTop: "1px solid var(--border-hex)", paddingTop: 10 }}>
            {hasLog && (
              <>
                    ...
                    {meals.map((m: any, i: number) => (
                      <div key={i} style={{ padding: "3px 0 3px 20px", fontSize: "0.7rem" }}>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <span style={{ color: "var(--muted-hex)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{m.name}</span>
                          <span style={{ color: "var(--muted-hex)", marginLeft: 8, whiteSpace: "nowrap" as const }}>{Math.round(m.kcal)} kcal</span>
                        </div>
                ...
                <div style={{ borderTop: "1px solid var(--border-hex)", marginTop: 8, paddingTop: 8 }} />
              </>
            )}

            <div style={{ fontSize: "0.7rem", fontWeight: 600, color: hasLog ? "var(--muted-hex)" : "var(--accent-hex)", marginBottom: 6 }}>
              📋 {hasLog ? "Planned" : "Plan"}
            </div>
            {day.meals.map((meal: any, i: number) => (
              <div key={i} style={{ padding: "5px 0", borderBottom: i < day.meals.length - 1 ? "1px solid var(--border-hex)" : "none" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: "0.8rem", fontWeight: 600 }}>
                    {getMealEmoji(meal.name)} {meal.name}
                  </span>
                  <span style={{ fontSize: "0.7rem", color: "var(--muted-hex)" }}>{meal.kcal} kcal</span>
                </div>
                {meal.items && (
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 12 }}>
        <CardContent style={{ padding: "var(--space-lg)" }}>
          <h2 style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--muted-hex)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "var(--space-md)" }}>📊 Weekly Overview</h2>
          <div style={{ display: "flex", justifyContent: "space-around", textAlign: "center" }}>
            {[
              ...
              <div key={m.label}>
                ...
                {m.plan != null && (
                  <div style={{ fontSize: "0.6rem", color: "var(--muted-hex)" }}>plan: {m.plan}{m.unit}</div>
                )}
                <div style={{ fontSize: "0.6rem", color: "var(--muted-hex)" }}>{m.label}/day</div>
              </div>
            ))}
          </div>
          {avgActual && (
            <div style={{ textAlign: "center", fontSize: "0.65rem", color: "var(--muted-hex)", marginTop: 6 }}>
      ...
      <div style={{
        textAlign: "center", color: "var(--muted-hex)", fontSize: "0.7rem",
        padding: "12px 0", borderTop: "1px solid var(--border-hex)", marginTop: 8,
      }}>
